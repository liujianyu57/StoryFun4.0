...
2 Portfolio：左 = Portfolio balance（大字 + “Across N open pons position(s)”小字）
             右 = PnL 走势卡：头部大字 PnL（±$，随 1D/7D/30D 切换）+ 折线图
3 领取横幅（仅本人、有可领内容时出现）：
   - Holder dividends ready（◎）+ Claim all holder dividends
   - Creator fees（◆）+ Claim fees
4 页签：Positions / History / Activity / Launches（下划线高亮当前）
5 内容区（随页签渲染；未登录时为登录引导卡）
6 分享浮层：HTML 分享卡（“看起来像图片”，仅示意，不生成文件）
```
